...
export const sendMessage = (chatId: string, message: string): ThunkAction<Return, IAppStore, ExtraArgument, {type: any}> =>
    async (dispatch: ThunkDispatch<IAppStore, ExtraArgument, {type: any}>, getStore: IGetStore) => {
        ...
        if (token) {
            ...
            try {
                const data = await PrivateChatAPI.sendMessage(token, chatId, message);
                if (data.error) {
